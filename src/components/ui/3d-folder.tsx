...
const ImageLightbox: React.FC<ImageLightboxProps> = ({
  ...
  const getInitialStyles = (): React.CSSProperties => {
    ...
    const targetWidth = Math.min(900, viewportWidth - 48);
  ...
  className="w-full h-full object-cover object-[center_30%] select-none"
          ...
          <div
            className={cn("px-6 sm:px-8 py-6 sm:py-7 bg-card border-t border-white/5")}
            style={{
              opacity: animationPhase === "complete" && !isClosing ? 1 : 0,
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              ...
              <button
                className={cn(
                  "flex items-center justify-center gap-2 px-6 py-3 text-sm font-bold uppercase tracking-widest text-primary-foreground bg-primary hover:brightness-110 rounded-xl shadow-lg shadow-primary/20 transition-all duration-300 hover:scale-105 active:scale-95",
                )}
              >
                <span>View</span>
...
export const AnimatedFolder: React.FC<AnimatedFolderProps> = ({
  ...
  const tabBg = gradient || "hsl(var(--primary))";
  const frontBg =
    ...
    <>
      ...
      >
        <div
          ...
          style={{
            background: gradient
              ? `radial-gradient(circle at 50% 70%, hsl(var(--accent)) 0%, transparent 70%)`
              : "radial-gradient(circle at 50% 70%, hsl(var(--accent)) 0%, transparent 70%)",
        ...
        <div className="text-center">
          ...
          >
            {subtitle ?? `${projects.length} ${projects.length === 1 ? "member" : "members"}`}
          </p>
        </div>
